...
void placement_route (t_ville* ville, t_info_BFS* info, t_construction* construction)
{
    ...
    {
        info->clique = 1;
        info->depart->x = mouse_x/TAILLE_CASE;
        info->depart->y = mouse_y/TAILLE_CASE;
    }
    if (mouse_unclick && info->clique)
    {
        info->arrive->x = mouse_x/TAILLE_CASE;
        info->arrive->y = mouse_y/TAILLE_CASE;
...
{
    int raccord=0;
    if (x>1 && ville->plateau[x-1][y]->bat->type=='r')
        raccord = 100;
    if (x< LARGEUR_PLATEAU-1 && ville->plateau[x+1][y]->bat->type=='r')
        raccord = raccord + 1;
    if (y>1 &&  ville->plateau[x][y-1]->bat->type=='r')
